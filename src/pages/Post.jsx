...
export default function Post({ post }) {
 ...
 `http://localhost:3000/api/moderator/poststatus/${post._id}`,
  ...
  try{
  const response = await fetch(`http://localhost:3000/api/feed/flag/${post._id}`,
  {method: "PUT"});
  const result = await response.json();
  if(response.status === 200) {
  ...
  try{
  const response = await fetch(`http://localhost:3000/api/moderator/deletepost/${post._id}`,
  {method: "DELETE"});
  const result = await response.json();
  if(response.status == 200) {
  toast.success(result.message);
  ...
  try{
  const response = await fetch(`http://localhost:3000/api/feed/like/${post._id}`,
  {method: "PUT"});
  const result = await response.json();
  if(response.status === 200){
  feeds.updatefeed(result);
          ...
          <div className="postcreator">
            ...
            {(currentuser.user.is_Admin && (
              <span className="moderator-options">
                <span> {post.status === "active" && (<i onClick={handlePostToggle} className="fa fa-solid fa-toggle-on text-success pe-auto  " aria-hidden="true"></i>) || (<i onClick={handlePostToggle} className="fa fa-solid fa-toggle-off pe-auto " aria-hidden="true"></i>)}
                  <i
                    onClick={handleModerator}
                    className="fa fa-times"
                    aria-hidden="true"
                  ></i></span>
                {post.flagCount.length && <i class="fa fa-flag text-danger pl-2" aria-hidden="true">
                  {" "}
          ...
          <div className="like-comment-container">
            ...
            >
              {(post.likeCount.indexOf(currentuser.user.user_id) !== -1 &&
                ...
                `${(post.likeCount.length && post.likeCount.length || '')} Like`}
              <i class="fa fa-thumbs-up m-2" aria-hidden="true"></i>
            </div>
            <div onClick={handleComment}>
              <button style={{ border: "none", backgroundColor: "white" }}>
                Comment<i className="fa fa-comment m-2" aria-hidden="true"></i>
              </button>
            </div>
