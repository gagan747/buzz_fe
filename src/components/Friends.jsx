...
try{
    ...
    console.log(err);
               ...
               setFriends(allfriends.current);
                ...
                {let namesplit=e.target.value.split(" ");
                  ...
                  regexlast=new RegExp(str1,"i");}
                    ...
                    if(e.target.value.indexOf(" ")!==-1){
                   if(regexfirst.test(friend.firstname) && regexlast.test(friend.lastname))
      ...
      <input  onChange={handleSearch} type="text" id="input" placeholder="Friends"/><label htmlFor='input'><i class="fa fa-search" aria-hidden="true"></i></label></div>
      {friends.map((friend)=>{            
            return (
               <div className="users-container">
                 <span> <img
                    ...
                    height="34px"
                  />  {"  "}
                  <span className="fullname">{friend.firstname + " " + friend.lastname}</span></span><span>
